...
**4. 프론트엔드 요청 직렬화를 완전히 제거한 상태로 재검증 — 동시 요청 20개 × 10회 반복,
총 200개 요청** (아래 "프론트엔드는 직렬화 불필요" 참고, 프론트 코드 개입 없이 백엔드에
curl로 직접 부하를 준 결과):
...
**세 단계 누적 총 350개 요청, 실패 0건.** 동시성을 5→10→20으로 계속 올려도 흔들림 없이
안정적이었다 — 백엔드 수정 하나만으로 충분하다는 게 확인됐다.

## 프론트엔드 — 요청 직렬화는 적용하지 않기로 결정

백엔드 수정 전에는 임시방편으로 `chulgunhaza-frontend`의 `src/api/client.ts`에 axios
요청 직렬화(동시 요청 1개로 제한)를 추가해봤었다. 하지만 위 200개 재검증 결과 백엔드
수정만으로 충분히 안정적임이 확인되어, **직렬화 코드는 제거하고 PR([#8](https://github.com/chulgunhaza/chulgunhaza-frontend/pull/8))도 닫았다.**
이유는 두 가지:

- 근본 원인이 없어졌으니 굳이 프론트에서 추가로 방어할 필요가 없다.
- 요청 직렬화는 페이지 로드 시 API 응답을 순차적으로 기다리게 만들어 체감 지연을
  늘리는 비용이 있다 — 이제 필요 없는 비용이라 뺐다.
