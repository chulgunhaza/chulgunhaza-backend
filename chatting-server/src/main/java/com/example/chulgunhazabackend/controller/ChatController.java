...
public class ChatController {
    ...
    public ResponseEntity<Void> createMessage(@Valid @RequestBody ChatMessageCreateRequestDto chatMessageCreateRequestDto, @AuthenticationPrincipal EmployeeCredentialDto employeeCredentialDto){
        // #101: 발신자 이름/사번은 클라이언트 입력이 아니라 JWT 클레임(로그인 인증 주체)에서
        // 가져온다 — 이걸 요청 바디로 받으면 다른 사람 이름을 사칭해서 보낼 수 있다(IDOR).
        chatRabbitMQMessageService.sendChatMessage(chatMessageCreateRequestDto, employeeCredentialDto.getId(), employeeCredentialDto.getName(), employeeCredentialDto.getEmployeeNo());
