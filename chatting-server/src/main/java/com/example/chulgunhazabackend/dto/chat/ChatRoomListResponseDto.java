...
public class ChatRoomListResponseDto {
    ...
    // INFO : otherMembers를 EmployeeChatRoom이 아니라 EmployeeSummary로 받는다 —
    // EmployeeChatRoom엔 이제 employeeId만 있고(#74 Phase 0), 이름/부서 같은 표시용
    // 데이터는 없어서 호출부(ChatRoomServiceImpl)에서 EmployeeDirectoryClient로
    // 미리 조회해 넘겨준다(#101: user-server 내부 API 호출, EmployeeRepository 직접
    // 참조는 chatting-server 물리 분리로 더 이상 불가능).
    public static ChatRoomListResponseDto fromEntity(Long roomId, List<EmployeeSummary> otherMembers, String lastMessage, Long unReadMessageCount, LocalDateTime lastMessageTime) {
