...
public class ChatRoomServiceImpl implements ChatRoomService {
    ...
    public Long saveChatRoom(ChatRoomCreateRequestDto chatRoomCreateRequestDto, Long employeeId) {
        ...
        // 여기서 직접 물지는 않으니(#74 Phase 0) 객체 전체가 아니라 존재 여부만 확인한다.
        // #101: chatting-server엔 EmployeeRepository가 없어서 user-server 내부 API를
        // 호출하는 EmployeeDirectoryClient로 대체한다.
        if (!employeeDirectoryClient.existsById(employeeId)) {
            throw new ChatException(ChatExceptionType.EMPLOYEE_NOT_FOUND);
        }
        for (Long id : memberIds) {
            if (!employeeDirectoryClient.existsById(id)) {
                throw new ChatException(ChatExceptionType.EMPLOYEE_NOT_FOUND);
            }
        }
    ...
    public PageDto<ChatRoomListResponseDto> getAllChatRoomsByEmployeeId(Long employeeId, Pageable pageable) {
        ...
        Page<ChatRoomListResponseDto> list = myRooms.map(myRoom -> {
            ...
            // 이름/부서 등은 여기서 EmployeeDirectoryClient로 한 번에 조회해 조합한다.
            // #101: EmployeeRepository.findAllById → user-server 내부 API 호출로 대체.
            List<Long> otherMemberIds = otherMembers.stream().map(EmployeeChatRoom::getEmployeeId).toList();
            List<EmployeeSummary> otherMemberSummaries = employeeDirectoryClient.findByIds(otherMemberIds);
