...
public class ChatMessageServiceImpl implements ChatMessageService {
    ...
    public Long saveChatMessage(ChatMessageCreateRMQDto chatMessageCreateRMQDto) {
        ...
        // #101: EmployeeRepository → EmployeeDirectoryClient(user-server 내부 API).
        Long senderId = chatMessageCreateRMQDto.getSenderId();
        if (!employeeDirectoryClient.existsById(senderId)) {
    ...
    private void broadcastReadReceipt(Long roomId, Long readerId, Long fromMessageId, Long toMessageId, List<EmployeeChatRoom> members) {
        ...
        // 어차피 정확한 값을 받는다. 읽음 영수증은 "지금 붙어있으면 갱신"이 목적이라, 완전히
        // 오프라인이면(deliverOrFanout이 false) SSE 폴백 없이 그냥 넘어간다 —
        // ChatRabbitMQMessageServiceImpl.deliverToOneReceiver와 다른 점.
        // #101: 로컬 세션 직접 조회 대신 ChatDeliveryService로 통일 — 다른 chatting-server
        // 인스턴스에 붙어있는 경우까지 커버한다(Redis Pub/Sub 팬아웃).
        for (EmployeeChatRoom member : members) {
            Long memberId = member.getEmployeeId();
            if (memberId.equals(readerId)) {
                continue;
            }
            chatDeliveryService.deliverOrFanout(memberId, roomId, event);
        }
    }
}
