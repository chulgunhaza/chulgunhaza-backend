...
class ChatMessageServiceImplTest {
    ...
    // #101: 로컬 WebSocket 세션 직접 조회(WebSocketMessageHandler) 대신 다른
    // chatting-server 인스턴스로의 팬아웃까지 판단하는 ChatDeliveryService로 통일됐다.
    @Mock
    ...
    void 읽으면_실시간으로_다른_참여자에게_알림이_간다() throws Exception {
        ...
        // 새로 읽힌 메시지의 발신자에게는 실시간 알림 시도가 가야 하고(#101: 로컬/팬아웃 판단은
        // ChatDeliveryService에 위임 — 여기선 "그 서비스를 호출했는지"만 확인한다),
        verify(chatDeliveryService).deliverOrFanout(org.mockito.ArgumentMatchers.eq(senderId), org.mockito.ArgumentMatchers.eq(roomId), org.mockito.ArgumentMatchers.any());
        // "읽은 나"(readerId) 본인에게는 자기 자신한테 보낼 이유가 없으니 전달 시도조차 안 해야 한다.
        org.mockito.Mockito.verify(chatDeliveryService, org.mockito.Mockito.never()).deliverOrFanout(org.mockito.ArgumentMatchers.eq(readerId), org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any());
    }
