...
class ChatMessageListenerTest {
    ...
    private ChatMessageCreateRMQDto dto() {
        return new ChatMessageCreateRMQDto(1L, "발신자", 1001L, "hello", 10L, LocalDateTime.now());
    }
    ...
    @Test
    @DisplayName("사원 예외(EMPLOYEE_NOT_FOUND)도 ChatException으로 통일되어 nack(no-requeue)로 처리된다")
    void 사원_예외는_기존대로_드롭된다() throws Exception {
        // #101: chatting-server엔 더 이상 EmployeeException(user-server 전용)이 없다 —
        // "존재하지 않는 사원" 판정은 ChatExceptionType.EMPLOYEE_NOT_FOUND로 통일됐다.
        willThrow(new ChatException(ChatExceptionType.EMPLOYEE_NOT_FOUND))
                .given(chatMessageService).saveChatMessage(any());
